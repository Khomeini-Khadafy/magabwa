...
<body class="font-[Poppins] pb-[83px]">
    ...
    <section id="author" class="max-w-[1130px] mx-auto flex items-center flex-col gap-[30px] mt-[70px]">
        ...
        <div id="content-cards" class="grid grid-cols-3 gap-[30px]">
            @forelse ($author->news as $author)
            <a href="{{ route('front.details', $author->slug) }}" class="card">
                <div
                    class="flex flex-col gap-4 p-[26px_20px] transition-all duration-300 ring-1 ring-[#EEF0F7] hover:ring-2 hover:ring-[#FF6B18] rounded-[20px] overflow-hidden bg-white">
                    <div class="thumbnail-container h-[200px] relative rounded-[20px] overflow-hidden">
                        <div
                            class="badge absolute left-5 top-5 bottom-auto right-auto flex p-[8px_18px] bg-white rounded-[50px]">
                            <p class="text-xs leading-[18px] font-bold">{{ $author->category->name }}</p>
                        </div>
                        <img src="{{ Storage::url($author->thumbnail) }}" alt="thumbnail photo"
                            class="w-full h-full object-cover" />
                    </div>
                    <div class="flex flex-col gap-[6px]">
                        <h3 class="text-lg leading-[27px] font-bold">
                            {{ substr($author->name, 0, 80) }}{{ strLen($author->name) > 80 ? '...' : '' }}
                        </h3>
                        <p class="text-sm leading-[21px] text-[#A3A6AE]"> 
                            {{ $author->created_at->format('M d, Y') }}</p>
                    </div>
                </div>
            </a>
            @empty
        ...
        <div class="flex flex-col gap-3 shrink-0 w-fit">
            ...
            <p class="font-medium text-sm leading-[21px] text-[#A3A6AE] flex gap-1">
                ...
                <a href="#" class="w-[18px] h-[18px]">
                    <img src="{{ asset('assets/images/icons/message-question.svg') }}" alt="icon" /></a>
            </p>
        </div>
    </section>
    {{-- End BannerAds --}}
